"""
based off of https://github.com/disruptops/resource-counter
"""
...
# some refactoring -> use session totals
# refactor the helper methods
...
def controller(access, secret, profile):
    ...
    region_list = resource_regions('ec2')
    for region in region_list:
        resource_counts[region] = {}
    resource_counts[global_region] = {}
...
def iam_counter():
    ...
    resource_counts[global_region]['Users'] = total_users
    resource_counts[global_region]['Groups'] = total_groups
    resource_counts[global_region]['Roles'] = total_roles
    resource_counts[global_region]['Policies'] = total_policies
    resource_counts[global_region]['SAML Providers'] = total_saml

def autoscaling_counter():
    # get list of supported regions
    region_list = resource_regions('autoscaling')
...
def balancer_counter():
    # get list of supported regions
    elb_region_list = resource_regions('elb')
    elbv2_region_list = resource_regions('elbv2')
...
def s3_counter():
    ...
    resource_counts[global_region]['s3 buckets'] = bucket_counter
    resource_totals['S3 Buckets'] = total_buckets

def lambda_counter():
    region_list = resource_regions('lambda')
...
def glacier_counter():
    region_list = resource_regions('glacier')
...
def cloudwatch_rules_counter():
    region_list = resource_regions('events')
...
def config_counter():
    region_list = resource_regions('config')
...
def cloudtrail_counter():
    region_list = resource_regions('cloudtrail')
...
def sns_counter():
    region_list = resource_regions('sns')
...
def sqs_counter():
    region_list = resource_regions('sqs')
    ...
    for region in region_list:
        ...
        queue_counter = len(list(queue_iterator))
        total_queues += queue_counter
        resource_counts[region]['sqs queues'] = queue_counter
    resource_totals['SQS Queues'] = total_queues

def kms_counter():
    region_list = resource_regions('kms')
...
def dynamo_counter():
    region_list = resource_regions('dynamodb')
...
def rds_counter():
    region_list = resource_regions('rds')
